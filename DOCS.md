...
`messages[*]` returns all elements in the messages table.
`sort message,asc` sorts that return by the column "message"
in ascending order. `where message,in,a` returns every element
in the "message" column which includes the string "a". `random 5`
returns 5 random values. `sort message,dsc` sorts those 5 values by
the "messages" row in descending order and then `random 2` returns 2
random values.

As you can see the full query is nonsensical and you could get the same result by
using this query instead:

`messages[*] | where message,in,a | random 2`
